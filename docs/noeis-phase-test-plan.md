...
- AT-328 now has local proof that pulled Think references enter the shared agent context and travel into promoted Wiki pages as source provenance; remaining risk is browser proof across all three postures after deploy.
- AT-330 has local route/read/workspace proof plus deterministic seeded Talk promotion proof; remaining risk is production-data proof after deploy.
- AT-324 has token, critical-CSS, reduced-motion, and local dark-mode Wiki browser proof; remaining risk is broader Home/Think visual review after deploy.
- AT-333 has selected-node cross-surface traces plus authenticated seeded promotion proof that creates a cited page with source and graph traces; remaining risk is production-data verification after deploy.
...
- Current browser proof: local `/library?scope=all` shows the shared Thought partner rail with one `agent-ticker`, `Thought partner library trace`, zero horizontal overflow, and no legacy `Context` title in the right rail; local Wiki and Think smoke checks still show shared agent presence/ticker, no legacy Wiki chat, and zero horizontal overflow. Local `/wiki/workspace?view=graph&pull=1` opens the chat/reference picker and shows seeded candidates, but this Browser runtime cannot complete the candidate click through selector, DOM-event, or mouse APIs, so full seeded reciprocal-click proof remains open.
