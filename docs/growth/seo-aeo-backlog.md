# Noeis SEO/AEO Backlog

## Core wedge pages
- Most note apps solve capture, not recall
- Readwise is not a second brain
- How serious readers turn highlights into concepts
- AI for reading without losing judgment
- Best second brain app for founders
- Best second brain app for researchers

## Product-adjacent comparison pages
- Second brain app vs notes app
- PKM AI vs chat-only AI tools
- Connected notes vs folders for research
- How to choose a research workflow app

## Activation support pages
- How to import your reading archive into Noeis
- Your first concept in Noeis
- Source-backed synthesis workflow in Noeis
- From saved article to draft in Noeis
