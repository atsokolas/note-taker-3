...
- **Importer:** *"Tomorrow morning, Noeis will have grown this while you slept."* → one toggle to enable continuous Readwise/Notion **auto-sync** (cadence). This is the morning-paper loop, established day one. **Scheduler confirmed:** `wikiScheduledMaintenanceWorker` runs background page refreshes from the server process; the remaining onboarding work is to expose the return promise and cadence honestly.
...
4. **Return hook wiring.** Surface the auto-sync cadence toggle and the extension-install nudge at the end of the arc. **Scheduler confirmed:** `server/services/wikiScheduledMaintenanceWorker.js` runs due wiki pages through normal maintenance every 6 hours by default, while `wikiBriefingService` remains an on-demand read model over the latest page state. The product work is to expose this clearly, not to fake a spinner or duplicate the worker.
