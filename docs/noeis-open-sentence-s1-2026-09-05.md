...
The same pocket can hold an earlier recorded wording beside today's line. **Then** is identity, not a reconstructed scene: the newest unpruned Wiki revision whose `before` still has that claim, or a claim-history fallback when revisions do not record a prior line. The live marked sentence stays Now. If that revision also recorded a quotation for the same source identity, and it differs from today's passage, it sits under Then. If that same snapshot bound a question or a notebook draft to the claim, that recorded line sits under Then as well — "Then you left this open", "Then you wrote". A user history note on the Then line can stand in for a draft when the snapshot has none. Missing earlier wording, a missing historical quotation, or missing contemporaneous work stays missing. A private draft cannot forge Then. Then work is not copied into today's question. Library has no claim revisions, so it stays silent.
