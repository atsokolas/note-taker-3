...
const cardIdentity = (card = {}) => clean(
  card.sourceKey
  || card.id
  || `${card.type}|${card.sourcePath}|${card.title}|${stripHtml(card.content)}`
);

const genericIdentity = (item = {}, type = 'item') => clean(
  item.id
...
const mergeCards = (localCards = [], remoteCards = []) => {
...
const mergeGenericTimeline = (localItems = [], remoteItems = [], kind = 'item') => {
  const merged = new Map();
...
const mergeWorkbenchStates = (localState, remoteState, choices = {}) => {
